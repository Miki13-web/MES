...
struct elemUniv {
    ...
    // Tablica 4 wskaźników na powierzchnie (Dół, Prawa, Góra, Lewa)
    Surface* surfaces[4];
    ...
    elemUniv(int npc) : npc(npc) {
        ...
        default:
            cerr << "Nieobsługiwana liczba punktów całkowania: " << npc << endl;
        ...
        // Obliczanie wartości pochodnych w każdym punkcie całkowania
        for (int i = 0; i < npc; i++) {
            double eta = eta_pc[i];
            double ksi = ksi_pc[i];
